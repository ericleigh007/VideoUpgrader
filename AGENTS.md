...
- **Context** (`context/`) — Reference material and domain knowledge
  - in here, requirements.md are the base requirements for the project
...
* They are originally broken down from `context/requirements.md`
...
**During Session:**
- Append notable events to today's log: `python memory/memory_write.py --content "event" --type event`
- Add facts to the database: `python memory/memory_write.py --content "fact" --type fact --importance 7`
- For truly persistent facts (always loaded), update MEMORY.md: `python memory/memory_write.py --update-memory --content "New preference" --section user_preferences`

**Search Memory:**
- Keyword search: `python memory/memory_db.py --action search --query "keyword"`
- Semantic search: `python memory/semantic_search.py --query "related concept"`
- Hybrid search (best): `python memory/hybrid_search.py --query "what does user prefer"`
...
6. Add integration tests for anything that would normally use the UI so that the user isn't required to feedback interactive test results

# **Stand-Alone testing -- NOT asking the user to check the GUI**
- On GUI-based apps, don't ask the user to try it.
- Keep an interface layer that can be tested WITHTOUT the GUI.
- Test it without the user.

# **Add Verfication of the GUI using GUI testing tools**
- Don't ask the user to test the GUI.
- Use automated GUI testing tools based on Playright or other frameworks.
- Build tests into the system for this.
